...
- POST送信の方式
  ...
  - Webフォームとcurlコマンドではボディを異なるフォーマットで変換する
    ...
    - curl: RFC3986(--data-urlencode指定時)
      - curlは通常(`-d`オプション)指定された文字列をそのまま接続する
      - キーと値は`=`、各項目を`&`で接続
...
- **マルチパートフォーム形式**
  ...
  - 一度のリクエストで複数のファイル送信が可能
    - 受け取り側でファイルを区切る必要がある
    - `Content-Type`にMIMEタイプとは別で区切り文字列(boundary:境界、しきり)の指定が必須
...
- フォーム送信時時のレスポンス
  ...
  - リダイレクトさせずに新しいページの内容をリクエストを送ってきたページのまま返す方法もある
